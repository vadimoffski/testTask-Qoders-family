...
let mapBtns = document.querySelectorAll(".department");
let mapTxts = document.querySelectorAll(".department__title");

let info_title = document.querySelector(".info__title");
let info_text = document.querySelector(".info__text");
let info_worktime = document.querySelector(".worktime__timetable");
let info_phone = document.querySelector(".phone__numbers");
let info_mail = document.querySelector(".mail__address");

function initMap(location) {
  let contactUs = {
    new_york: [
      { lat: 40.71771464219023, lng: -74.00999811028004 },
      {
        title: "NewYork",
        text: "2005 Stokes Isle Apt. 896, Venaville, New York 10010",
        worktime: "Mon - Sat: 11:00-19:00, Sun:11:00-16:00,",
        phone: "(0043) 568 456 902,<br /> (0043) 568 456 902",
        mail: "hello@pad.architecture, support@pad.architecture",
      },
    ],
    los_angeles: [
      { lat: 34.064914025071204, lng: -118.24323644116224 },
      {
        title: "Los Angeles",
        text: "2005 Stokes Isle Apt. 896, Venaville, New York 10010",
        worktime: "Mon - Sat: 11:00-19:00, Sun:11:00-16:00,",
    ...
    boston: [
      { lat: 42.38150361667544, lng: -71.06095341604896 },
      {
        title: "Boston",
        text: "2005 Stokes Isle Apt. 896, Venaville, New York 10010",
        worktime: "Mon - Sat: 11:00-19:00, Sun:11:00-16:00,",
    ...
    detroit: [
      { lat: 42.341968620543604, lng: -83.04565714220185 },
      {
        title: "Detroit",
        text: "2005 Stokes Isle Apt. 896, Venaville, New York 10010",
        worktime: "Mon - Sat: 11:00-19:00, Sun:11:00-16:00,",
  ...
  let opt = {
    mapId: "3ee4c9489dffc2e1",
    center: location || contactUs.new_york[0],
    zoom: 12,
  ...
  const marker = new google.maps.Marker({
    position: location || contactUs.new_york[0],
    map: map,
    icon: "img/icons/location.svg",
    animation: google.maps.Animation.DROP,
  });

  function replaceIInfoBody(titleButton) {
    for (key in contactUs) {
      let city = contactUs[key][1];
      if (city.title === "Los Angeles" && titleButton === "Los Angeles, USA") {
        info_title.innerHTML = `${city.title}`;
        info_text.innerHTML = `${city.text}`;
        info_worktime.innerHTML = `${city.worktime}`;
        info_phone.innerHTML = `${city.phone}`;
        info_mail.innerHTML = `${city.mail}`;
      } else if (city.title === "NewYork" && titleButton === "NewYork, USA") {
        info_title.innerHTML = `${city.title}`;
        info_text.innerHTML = `${city.text}`;
        info_worktime.innerHTML = `${city.worktime}`;
        info_phone.innerHTML = `${city.phone}`;
        info_mail.innerHTML = `${city.mail}`;
      } else if (city.title === "Boston" && titleButton === "Boston, USA") {
        info_title.innerHTML = `${city.title}`;
        info_text.innerHTML = `${city.text}`;
        info_worktime.innerHTML = `${city.worktime}`;
        info_phone.innerHTML = `${city.phone}`;
        info_mail.innerHTML = `${city.mail}`;
      } else if (city.title === "Detroit" && titleButton === "Detroit, USA") {
        info_title.innerHTML = `${city.title}`;
        info_text.innerHTML = `${city.text}`;
        info_worktime.innerHTML = `${city.worktime}`;
        info_phone.innerHTML = `${city.phone}`;
  ...
  mapBtns.forEach((mapBtn) => {
    mapBtn.addEventListener("click", function () {
      mapBtns.forEach((i) => i.classList.remove("_activeMap"));
      this.classList.toggle("_activeMap");
      let titleButton = this.querySelector(".department__title").textContent;
      if (titleButton === "Los Angeles, USA") {
        let location = contactUs.los_angeles[0];
        initMap(location);
        replaceIInfoBody(titleButton);
      } else if (titleButton === "NewYork, USA") {
        let location = contactUs.new_york[0];
        initMap(location);
        replaceIInfoBody(titleButton);
      } else if (titleButton === "Boston, USA") {
        let location = contactUs.boston[0];
        initMap(location);
        replaceIInfoBody(titleButton);
      } else if (titleButton === "Detroit, USA") {
        let location = contactUs.detroit[0];
        initMap(location);
        replaceIInfoBody(titleButton);
      }
    });
...
function scrollFunction() {
  if (document.body.scrollTop > 80 || document.documentElement.scrollTop > 82) {
    document.querySelector(".header__body").style.minHeight = "40px";
    document.querySelector(".header__wrapper").style.backgroundColor =
      "#e9e9e9";
    document.querySelector(".page__services").style.marginTop = "40px";
    document.querySelector(".page__contact-us").style.paddingTop = "40px";
  } else {
    document.querySelector(".header__body").style.minHeight = "82px";
    document.querySelector(".header__wrapper").style.backgroundColor =
      "transparent";
  }
}
